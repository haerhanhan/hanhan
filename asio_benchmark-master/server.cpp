    ...
    void read() {
        ...
        socket_.async_read_some(asio::buffer(buffer, block_size_),
            ...
            if (!err && verify_crc8(buffer,cb)) {
               ...
               //std::cout<<buffer<<"\n";
               //uint8_t prev_crc = buffer[cb-1];
                //auto crc_again = crc8_continue(prev_crc,buffer,cb);
                 //std::cout<<std::hex<<(unsigned int )(unsigned char)prev_crc <<"\n" ;
                // std::cout<<std::hex<<(unsigned int )(unsigned char)crc_again <<"\n" ;
...
public:
    server(int thread_count, const asio::ip::tcp::endpoint& endpoint,
        ...
        , block_size_(block_size)
        , service_pool_(thread_count)//建立线程池
        , acceptor_(service_pool_.get_io_service())//acceptor(io_service)
    {
...
private:
    void accept() {
        ...
        acceptor_.async_accept(socket,
            ...
            if (!err) {
                new_session->start();//不需要计算总的session传输数据，所以不用建立session的vector
                accept();
            }
        });
    }
